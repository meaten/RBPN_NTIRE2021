    ...
    def __init__(self):
        ...
        class Decoder(torch.nn.Module):
            ...
            def backwarp(self, tenInput, tenFlow):
                if str(tenFlow.shape) + str(tenFlow.device) not in self.backwarp_tenGrid:
                    tenHor = torch.linspace(-1.0 + (1.0 / tenFlow.shape[3]), 1.0 - (1.0 / tenFlow.shape[3]), tenFlow.shape[3]).view(1, 1, 1, -1).expand(-1, -1, tenFlow.shape[2], -1)
                    tenVer = torch.linspace(-1.0 + (1.0 / tenFlow.shape[2]), 1.0 - (1.0 / tenFlow.shape[2]), tenFlow.shape[2]).view(1, 1, -1, 1).expand(-1, -1, -1, tenFlow.shape[3])

                    self.backwarp_tenGrid[str(tenFlow.shape) + str(tenFlow.device)] = torch.cat([tenHor, tenVer], 1).to(tenFlow.device)

                if str(tenFlow.shape) + str(tenFlow.device) not in self.backwarp_tenPartial:
                    self.backwarp_tenPartial[str(tenFlow.shape) + str(tenFlow.device)] = tenFlow.new_ones([ tenFlow.shape[0], 1, tenFlow.shape[2], tenFlow.shape[3] ])

                tenFlow = torch.cat([ tenFlow[:, 0:1, :, :] / ((tenInput.shape[3] - 1.0) / 2.0), tenFlow[:, 1:2, :, :] / ((tenInput.shape[2] - 1.0) / 2.0) ], 1)
                tenInput = torch.cat([ tenInput, self.backwarp_tenPartial[str(tenFlow.shape) + str(tenFlow.device)] ], 1)

                tenOutput = torch.nn.functional.grid_sample(input=tenInput, grid=(self.backwarp_tenGrid[str(tenFlow.shape) + str(tenFlow.device)] + tenFlow).permute(0, 2, 3, 1),
